...
<script lang="ts" setup>
  ...
  import { formSchema, procurementOrderItemColumns } from '../ProcurementOrderSupplier.data';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (unref(isUpdate)) {
      ...
      requestSubTableData(procurementOrderItemList, { id: data?.record?.id }, procurementOrderItemTable);
    }
    // 隐藏底部时禁用整个表单
  ...
  async function reset() {
    await resetFields();
    activeKey.value = 'procurementOrderItem';
